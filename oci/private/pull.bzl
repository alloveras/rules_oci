...
load("@rules_oci//oci:platform.bzl", "oci_pulled_image")
...
def _oci_pull_impl(rctx):
    ...
    config = json.decode(rctx.read(config_output_path))

    # Derive OCIPlatformInfo fields:
    #  - When the caller supplied 'platform', validate it matches the values in the image config.
    #  - When the caller has NOT supplied 'platform', extract the information from the image config.
    if rctx.attr.platform:
        util.validate_image_platform(rctx, config)
        plat_os, plat_cpu, plat_variant = util.platform_triplet(rctx.attr.platform)
    else:
        plat_os = config["os"]
        plat_cpu = config["architecture"]
        plat_variant = config.get("variant")
    ...
    attrs = dicts.add(
        _IMAGE_REFERENCE_ATTRS,
        {
            "platform": attr.string(
                doc = "A single platform in `os/arch` or `os/arch/variant` format, used when pulling a specific architecture from a multi-arch image. The generated target provides `OCIPlatformInfo` with `os`, `cpu`, and `variant` populated from this value.",
            ),
