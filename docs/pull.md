...
| <a id="oci_pull-platforms"></a>platforms |  a list of the platforms the image supports, each in `os/arch` or `os/arch/variant` format. Mandatory for multi-architecture images. Optional for single-architecture images. When provided, this creates a separate external repository for each platform, avoiding fetching layers, and an alias that validates the presence of an image matching the target platform's cpu. Each per-platform target provides `OCIPlatformInfo` with the `os`, `cpu`, and `variant` fields populated from the platform string. When omitted for a single-architecture image, `OCIPlatformInfo` is still provided, with fields populated from the image config metadata.   |  `None` |
